package org.project.onboarding.predicates.Task;
...
public class TaskPredicate {
    ...
    public TaskPredicate(SearchCriteria criteria) {
        this.criteria = criteria;
    }

    public BooleanExpression getPredicate() throws BusinessException {
        PathBuilder<Task> entityPath = new PathBuilder<>(Task.class, "task");

        String key = criteria.getKey();
        String operation = criteria.getOperation();
        Object value = criteria.getValue();
        if (!key.equalsIgnoreCase("isDeleted") && !isValidKey(key)) {
            throw new BusinessException(String.format("Key %s does not exist in Task table.", key), HttpStatus.BAD_REQUEST);
        }
    ...
    private boolean isValidKey(String key) {
        List<Field> fields = getAllFieldsIncludingInherited(ListTaskDTO.class);
