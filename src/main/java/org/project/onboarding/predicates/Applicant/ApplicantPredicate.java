package org.project.onboarding.predicates.Applicant;
...
@Data
public class ApplicantPredicate {

    private SearchCriteria criteria;

    public ApplicantPredicate(SearchCriteria criteria) {
        this.criteria = criteria;
    }

    public BooleanExpression getPredicate() throws BusinessException {
        PathBuilder<Applicant> entityPath = new PathBuilder<>(Applicant.class, "applicant");

        String key = criteria.getKey();
        String operation = criteria.getOperation();
        Object value = criteria.getValue();
        if (!key.equalsIgnoreCase("isDeleted") && !isValidKey(key)) {
            throw new BusinessException(String.format("Key %s does not exist in Applicant table.", key), HttpStatus.BAD_REQUEST);
        }
    ...
    private boolean isValidKey(String key) {
        List<Field> fields = getAllFieldsIncludingInherited(ListApplicantsResponseDTO.class);
