...
import org.project.onboarding.dto.teamMember.ListTeamMemberDTO;
...
public class TeamMemberPredicate {

    private SearchCriteria criteria;

    public TeamMemberPredicate(SearchCriteria criteria) {
        this.criteria = criteria;
    }

    public BooleanExpression getPredicate() throws BusinessException {
        PathBuilder<TeamMember> entityPath = new PathBuilder<>(TeamMember.class, "teamMember");

        String key = criteria.getKey();
        String operation = criteria.getOperation();
        Object value = criteria.getValue();
        if (!key.equalsIgnoreCase("isDeleted") && !isValidKey(key)) {
            throw new BusinessException(String.format("Key %s does not exist in TeamMember table.", key), HttpStatus.BAD_REQUEST);
        }
    ...
    private boolean isValidKey(String key) {
        List<Field> fields = getAllFieldsIncludingInherited(ListTeamMemberDTO.class);
