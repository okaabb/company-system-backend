package org.project.onboarding.predicates.HiringPhase;
...
@Data
public class HiringPhasePredicate {

    private SearchCriteria criteria;

    public HiringPhasePredicate(SearchCriteria criteria) {
        this.criteria = criteria;
    }

    public BooleanExpression getPredicate() throws BusinessException {
        PathBuilder<HiringPhase> entityPath = new PathBuilder<>(HiringPhase.class, "HiringPhase");

        String key = criteria.getKey();
        String operation = criteria.getOperation();
        Object value = criteria.getValue();
        if (!key.equalsIgnoreCase("isDeleted") && !isValidKey(key)) {
            throw new BusinessException(String.format("Key %s does not exist in HiringPhase table.", key), HttpStatus.BAD_REQUEST);
        }
    ...
    private boolean isValidKey(String key) {
        List<Field> fields = getAllFieldsIncludingInherited(ListHiringPhaseDTO.class);
