package org.project.onboarding.predicates.PhaseInterviewer;
...
@Data
public class PhaseInterviewerPredicate {

    private SearchCriteria criteria;

    public PhaseInterviewerPredicate(SearchCriteria criteria) {
        this.criteria = criteria;
    }

    public BooleanExpression getPredicate() throws BusinessException {
        PathBuilder<PhaseInterviewer> entityPath = new PathBuilder<>(PhaseInterviewer.class, "PhaseInterviewer");

        String key = criteria.getKey();
        String operation = criteria.getOperation();
        Object value = criteria.getValue();
        if (!key.equalsIgnoreCase("isDeleted") && !isValidKey(key)) {
            throw new BusinessException(String.format("Key %s does not exist in PhaseInterviewer table.", key), HttpStatus.BAD_REQUEST);
        }
    ...
    private boolean isValidKey(String key) {
        List<Field> fields = getAllFieldsIncludingInherited(ListPhaseInterviewerDTO.class);
