...
import org.project.onboarding.dto.training.ListTrainingDTO;
...
public class TrainingPredicate {
    ...
    public TrainingPredicate(SearchCriteria criteria) {
        this.criteria = criteria;
    }

    public BooleanExpression getPredicate() throws BusinessException {
        PathBuilder<Training> entityPath = new PathBuilder<>(Training.class, "training");

        String key = criteria.getKey();
        String operation = criteria.getOperation();
        Object value = criteria.getValue();
        if (!key.equalsIgnoreCase("isDeleted") && !isValidKey(key)) {
            throw new BusinessException(String.format("Key %s does not exist in Training table.", key), HttpStatus.BAD_REQUEST);
        }
    ...
    private boolean isValidKey(String key) {
        List<Field> fields = getAllFieldsIncludingInherited(ListTrainingDTO.class);
